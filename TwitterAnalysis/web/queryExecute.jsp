...
<s:select label=" Your Queries"
		...
		listKey="idQue"
		name="selectedQuery" />
<s:url id="newQuery" namespace="/" action="preQueryCreate" >
<s:param name="idCampaign" value="%{campaign.idCampaign}" />
</s:url>
<s:a href="%{newQuery}">Create New Query</s:a>
<s:submit></s:submit>
</s:form>

<s:form action="deleteQueryCreate">
<s:hidden name="idCampaign" value="%{idCampaign}"/>
<s:select label=" Delete Query"
		key="selectedQuery" 
		headerKey="-1" headerValue="Select query"
		list="queries" 
		listValue="queText"
